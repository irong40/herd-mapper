...
export default function StatusBar({
  isScouting, drone,
  waypoints, pendingAlerts, pass2Ready, scoutComplete, launchState,
  isLive, missionId, propertyName,
  clusterCount, totalAnimals, obstacleCount,
}) {
  const statusLabel = isScouting
    ? (scoutComplete ? 'OBSTACLE PROFILE READY' : 'SCOUT PASS IN PROGRESS')
    : launchState === 'executing' ? 'PASS 2 — EXECUTING'
    : launchState === 'complete'  ? 'PASS 2 — COMPLETE'
    : 'PASS 1 COMPLETE'

  const droneLabel = drone ? `· ${drone}` : ''
  ...
  return (
    <div className="h-11 bg-[#0a0d12]/95 border-b border-slate-800 flex items-center px-4 gap-5 shrink-0" style={{ backdropFilter: 'blur(8px)' }}>
      <div className="flex items-center gap-2">
        <DroneIcon size={16} color={
          launchState === 'executing' ? '#f59e0b'
          : scoutComplete            ? '#22c55e'
          : isScouting               ? '#f59e0b'
          : '#22c55e'
        } />
        ...
        <span className="text-[10px] text-slate-600 font-mono ml-1 uppercase tracking-widest">
          {statusLabel} {droneLabel}
        </span>
        {isLive
          ? <span className="text-[9px] font-mono px-1.5 py-0.5 rounded border border-green-800/60 text-green-500 uppercase tracking-widest">LIVE · {propertyName || missionId}</span>
      ...
      {isScouting ? (
        <Stat label="OBSTACLES" value={obstacleCount} color="text-amber-400" icon={<WaypointIcon size={13} color="#f59e0b" />} />
      ) : (
        <>
          <Stat label="CLUSTERS" value={clusterCount}  color="text-green-400"  icon={<DeerClusterIcon size={13} color="#22c55e" />} />
          <Stat label="ANIMALS"  value={totalAnimals}  color="text-green-300"  icon={<DeerClusterIcon size={13} color="#86efac" />} />
      ...
      {isScouting && scoutComplete && (
        <div className="ml-auto flex items-center gap-2 rounded px-3 py-1 border border-green-800/60 bg-green-950/30">
          ...
          <span className="text-[11px] text-green-400 font-mono uppercase tracking-wide">
            Obstacle profile saved · {obstacleCount} obstacles mapped
          </span>
        </div>
      )}

      {!isScouting && pass2Ready && !launchState && (
        <div className="ml-auto flex items-center gap-2 rounded px-3 py-1 border border-green-800/60 bg-green-950/30">
          <span className="w-1.5 h-1.5 rounded-full bg-green-400 animate-pulse inline-block" />
          <span className="text-[11px] text-green-400 font-mono uppercase tracking-wide">
